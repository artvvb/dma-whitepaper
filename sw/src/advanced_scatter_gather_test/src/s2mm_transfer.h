...
void S2mmAttachBuffer (S2mmTransferHierarchy *InstPtr, UINTPTR Buffer, u32 BufferLength) {
	...
	NumBds = RoundUpDivide(BufferLength * sizeof(u32), InstPtr->MaxBurstLengthBytes);
	BdSpaceBytes = (sizeof(XAxiDma_Bd) * (NumBds)) + (XAXIDMA_BD_MINIMUM_ALIGNMENT-sizeof(u32));
	...
	Status = XAxiDma_BdRingCreate(RingPtr, (UINTPTR)(BdSpaceAligned), (UINTPTR)(BdSpaceAligned), XAXIDMA_BD_MINIMUM_ALIGNMENT, NumBds);
	if (Status != XST_SUCCESS) {
...
void S2mmUnattachBuffer(S2mmTransferHierarchy *InstPtr) {
